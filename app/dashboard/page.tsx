...
export default async function DashboardPage() {
  ...
  const [jobs, facts, companies, sessions, actedOn] = await Promise.all([supabase.from("jobs").select("status, scanned_at").order("scanned_at", { ascending: false }), supabase.from("facts").select("status"), supabase.from("companies").select("id, name, ats_last_status"), supabase.from("sessions").select("model_costs, created_at").gte("created_at", new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString()), supabase.from("jobs").select("scanned_at, updated_at").in("status", ["interested", "applied"])]);
  const error = jobs.error ?? facts.error ?? companies.error ?? sessions.error ?? actedOn.error; if (error) return <ErrorState message={error.message} />;
